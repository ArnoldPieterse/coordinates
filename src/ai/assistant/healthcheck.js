...
const HEALTH_CHECK_PORT = process.env.AI_ASSISTANT_PORT || 3001;
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200 && response.status === 'healthy') {
            ...
            console.error('Health check failed: Invalid response', response);
            reject(new Error('Health check failed: Invalid response'));
          }
        } catch (error) {
          console.error('Health check failed: Invalid JSON response', error);
